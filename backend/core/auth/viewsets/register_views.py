...
class RegisterViewSet(ViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        serializer = self.serializer_class(data=request.data) #creates a serializer instance using incoming request data (request.data) — preparing it for validation and object creation or update.
        serializer.is_valid(raise_exception= True)#This validates the incoming data; if invalid, it automatically raises a 400 Bad Request with error details —
        user = serializer.save() #This creates and saves a new user instance using the validated data from the serializer — calling its create() or update() method under the hood.
        refresh = RefreshToken.for_user(user) #Generates a new refresh token (and linked access token) for the given user
        ...
        res = { #This creates a response dictionary res containing:
            ...
            "access" :str(refresh.access_token),#the access token as a string (used to authenticate API requests)
        }
        return Response({
            "user":serializer.data, #This includes the serialized user data (like username, email, etc.) in the API response — it's the newly created or logged-in user's details, structured as JSON.password will not appear in the serialized response (serializer.data), because it's write-only.
            "refresh":res["refresh"],
            "access": res["access"],
